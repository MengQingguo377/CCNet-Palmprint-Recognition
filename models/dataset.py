...
class NormSingleROI(object):
    ...
    def __call__(self, tensor):
        ...
        idx = tensor > 0
        t = tensor[idx] # 提取tensor非零值

        # print(t)
        m = t.mean()
        s = t.std() 
        t = t.sub_(m).div_(s+1e-6) # 将非零值转化为标准正态分布
        tensor[idx] = t # 将非零值替换回tensor
        ...
        if transforms is None:
            if not train: 
                ...
                self.transforms = T.Compose([  
                    ...
                    T.ToTensor(), # 将像素值从 [0, 255] 范围缩放到 [0, 1] 范围
                    NormSingleROI(outchannels=self.chs) # 将非零像素值处理为标准正态分布
                    ])
